...
export async function PATCH(request: Request, { params }: { params: Promise<{ replyId: string }> }) {
  ...
  if (body.action === 'delete') {
    const { error } = await supabase.from('kautilya_forum_replies').update({ deleted_at: new Date().toISOString() }).eq('id', replyId).eq('author_id', user.id)
  ...
  if (body.action === 'report') {
    ...
    const { error } = await supabase.from('kautilya_forum_reports').insert({ reporter_id: user.id, reply_id: replyId, reason: parsed.data.reason })
  ...
  if (body.action === 'hide' && canModerateForum(user.email, process.env.ADMIN_EMAIL)) {
    const { error } = await createAdminClient().from('kautilya_forum_replies').update({ hidden_at: new Date().toISOString() }).eq('id', replyId)
